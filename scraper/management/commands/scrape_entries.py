...
class Command(BaseCommand):
    help = "Scrape future/current race cards (entries, distance, surface, post times)."
    ...
    def handle(self, *args, **options):
        ...
        job = service.scrape_entries(race_date=race_date, track_codes=tracks)
        self.stdout.write(
            self.style.SUCCESS(
                f"[{job.status}] entries {race_date}: {job.races_upserted} races "
                f"(source={job.source})"
            )
        )
